...
simul.infect <- function(N.pop, beta, recovery, death, birth, days){
  ...
  #add an adjust factor (10*rbeta(1000,0.1,0.9)) (temporary) for p1;
  
  dur_infect<-ceiling(1/recovery)
  res <- matrix(NA,nrow=days+30,ncol=dur_infect)
  res[1,1] <- rbinom(1,N.pop,p1*(10*rbeta(1000,0.1,0.9)))
  for (i in 2:(days+30)){
          res[i,1] <- rbinom(1,N.pop,p1*(10*rbeta(1000,0.1,0.9)))
...
shed <- function(N.shed,r,p,days){
        ...
        for (i in 1:days){
                #overload N.shed
                N.shed <- ceiling(10^(runif(1,4,10)))
...
ex_decay <- function(n0, time, decay_rate = (log(0.001)/-24)){
  return(rbinom(length(n0), n0, exp(-decay_rate * time)))
}
...
#### calculate the next point of the pathogen journey to the end point
#p_liquid is the percent of liquid waste went into the drain;
#we can add some adjustment factor
# adjust_fac<-rbeta(length(data$N0),1,5)

N_next<-function(N0,T,p_liquid=0.5,adj_fac=1){
        #T is the time to reach point X from N0;
        time<-T%%24
        day<-T%/%24
        #N_flow number of pathogen will reach point X, if there is no decay;
        N_flow<-c(rep(0,day),N0[1:(length(N0)-day)])*(24-time)/24 + c(rep(0,day+1),N0[1:(length(N0)-day-1)])*time/24
        N_decay<-ex_decay(round(N_flow*p_liquid*adj_fac),T)
        return(N_decay)
}
